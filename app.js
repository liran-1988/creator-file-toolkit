...
function renderChecks(result) {
  ...
  for (const check of result.checks) {
    ...
    indicator.setAttribute("aria-label", check.status);
    ...
    copy.append(label, actual, expected);
...
async function inspectFile(file) {
  ...
  if (source.width * source.height > MAX_PIXELS) {
    ...
    throw new Error("This image is too large to process safely in a browser. Use an image below 40 megapixels.");
  }
...
async function correctImage() {
  ...
  try {
    ...
    elements.fixButton.disabled = false;
    setWorkspaceState("ready");
    setStatus(result.status === "pass" ? "Corrected thumbnail is ready to download." : result.summary, result.status);
  } catch (error) {
    elements.fixButton.disabled = false;
    showError(error);
  }
...
elements.sampleButton.addEventListener("click", async () => {
  try {
    await inspectFile(await createSampleFile());
  } catch (error) {
    showError(error);
  }
});
...
for (const button of elements.previewModes) {
  button.addEventListener("click", () => {
    ...
    elements.previewFrame.dataset.mode = button.dataset.previewMode;
  });
}
